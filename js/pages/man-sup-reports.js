...
// ===================================
// ===================================
const show_report_app = (report_data, btn_data) => {
    // console.log(JSON.stringify(report_data));
    // console.log(JSON.stringify(report_data?.app_data));
    ...
    let app_type = report_data?.app_type;
    let app_filter = report_data?.app_filter;
    let app_time_period = report_data?.app_time_period;
    let app_data = report_data?.app_data;
    let date_end = report_data?.date_end;
    let date_start = report_data?.date_start;

    let chartTitletext = app_type == 2 ? "Job Posts" : (app_type == 3 ? "Job Orders" : "Job Postings");

    chartTitletext = app_filter == 1 ? "All "+chartTitletext : (
        app_filter == 2 ? "Completed "+chartTitletext : (
            app_filter == 3 ? "Cancelled "+chartTitletext : (
                app_filter == 4 ? "Ongoing "+chartTitletext : (
                    app_filter == 5 ? "Expired "+chartTitletext : chartTitletext
                )
            )
        )
    );

    chartTitle.innerText = app_time_period == 3 ? "Monthly "+chartTitletext : "Daily "+chartTitletext;

    app_data.forEach(el=>{
        if(app_time_period == 2){

        } else if (app_time_period == 3){
    ...
    data={};
    let subTitleText = "From "+date_start+" to "+date_end;
...
$("#form-app").validate({
    ...
    submitHandler: function(form, event) { 
        ...
        data["app_type"] = formData?.app_type;
        data["app_filter"] = formData?.app_filter;
        data["app_time_period"] = formData?.app_time_period;
        data["date_start"] = formData?.date_start;
        data["date_end"] = formData?.date_end;
        ...
        $.ajax({
            type : 'POST',
            url : "http://localhost/slim3homeheroapi/public/ticket/get-orders-report", // DEV
            data : data,
            success : function(response) {
                // console.log("Your response after submission is:");
                // console.log("Response JSON: "+response);
